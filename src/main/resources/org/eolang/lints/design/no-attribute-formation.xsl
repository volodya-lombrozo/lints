  ...
  <xsl:template match="/">
    <defects>
      <xsl:if test="not(/object[metas/meta[head='tests']])">
        <xsl:for-each select="//o[eo:abstract(.) and not(parent::o[eo:abstract(.) and o[@base='∅']]) and not(@name = 'λ' and parent::o[eo:atom(.)]) and not(parent::o[@base='Q.org.eolang.bytes']) and not(o[@base='∅'])]">
          <defect>
