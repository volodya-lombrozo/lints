  ...
  <xsl:template match="/">
    <defects>
      <xsl:if test="not(/object[metas/meta[head='tests']])">
        ...
        <xsl:for-each select="//o[eo:abstract(.) and @name and not(ancestor::o[eo:abstract(.) and @name and @name != $primary]) and not(ancestor::o[o[@base='∅']]) and not(@name = 'λ' and parent::o[eo:atom(.)]) and not(parent::o[@base='Q.org.eolang.bytes']) and not(o[@base='∅'])]">
          <defect>
